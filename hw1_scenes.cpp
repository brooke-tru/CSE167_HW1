...
Matrix3x3 parse_transformation(const json &node) {
    ...
    for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
        if (auto scale_it = it->find("scale"); scale_it != it->end()) {
            ...
            Matrix3x3 translateMatrix = Matrix3x3::identity();
            translateMatrix(0,2) = translate.x;
            translateMatrix(1,2) = translate.y;
            F = translateMatrix * F;
            ...
            Matrix3x3 shearXMatrix = Matrix3x3::identity();
            shearXMatrix(0,1) = shear_x;
            F = shearXMatrix * F;
            ...
            Matrix3x3 shearYMatrix = Matrix3x3::identity();
            shearYMatrix(1,0) = shear_y;
            F = shearYMatrix * F;
        }
    }
    return F;
